...
if (! empty ( $_POST )) {
	...
	$insert = '';
	if (empty ( $_GET ['id'] )) {
		$insert = "INSERT INTO tavern_traits (trait, type) VALUES ('" . $_POST ['trait'] . "','" . $_POST ['type'] . "');";
	} 

	else {
		$insert = "UPDATE tavern_traits SET trait='" . $_POST ['trait'] . "', type='" . $_POST ['type'] . "' WHERE id=" . $_GET ['id'] . ";";
	}
	try {
		$db->query ( $insert );
		$added = true;
	...
	if (empty ( $_GET ['id'] )) {
		...
		$db = connect ();
		$query = "SELECT * FROM tavern_traits WHERE id='" . $_GET ['id'] . "';";
		try {
	...
	<div class="col-sm-4">
		<div
			...
			<div class="panel-heading">
				<div class="panel-title">Edit Tavern Trait</div>
			</div>
